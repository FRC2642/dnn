...
public class VisionClient extends Thread {
    ...
    public static String getDataString(HttpClient client, HttpRequest request) {
      try {
        HttpResponse<String> response =client.send(request, HttpResponse.BodyHandlers.ofString());
        String response_string = response.body();
    ...
    public void run() {
      try {
        URI uri = new URI("http://10.99.97.67:8080");
          ...
          while (true) {
            ...
            Robot.writeString(getDataString(client, request));
          }
